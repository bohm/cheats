...
\obs{Efektivní konstrukce simple množiny} Definujme predikát $Q(x,y)$:
...
\thm{První} $f \in ČRF_1 \then \exists a \st \forall x: \varphi_{f(a)}(x) \daeq \varphi_a(x)$.
...
\thm{Druhá} $f \in ČRF_1 \then \exists {\rm\ rostoucí\ } g \in PRF_1 \st \forall x: \varphi_{f(g(j))}(x) \simeq \varphi_{g(j)}(x)$.

\prf{} $g(j) = s_2(z,z,j)$, vol $z = e$, podobně jako první věta.

\thm{Třetí, pro více proměnných}
$f \in ČRF_{n+1} \then \exists a \forall x_{[1,n]}: \varphi_a(x_{[1,n]}) \simeq f(a,x_{[1,n]})$.

\prf{}
\itemize\ibull
\: Jedna proměnná: $f(y,x) \simeq \Psi_2(e,y,x) \simeq \Psi_1(s_1(e,y),x) \simeq \varphi_{s_1(e,y)}(x)$.
Vol $g(y) = s_1(e,y)$, použij první větu.
\: více proměnných: $f(y,\mls x n ) \simeq \Psi_{n+1}(e,y,\mls x n ) \simeq \Psi_n(s_1(e,y), \mls x n ) \simeq
\varphi_{s_1(e,y)}(\mls x n )$. $s_1(e,y)$ má povný bod $a$, vol $y = a$.
\endlist

\thm{Čtvrtá} $f \in ČRF_{n+1} \then \exists g \in PRF_n \st \forall x,\mls y n : \varphi_{f(g(\mls y n ), \mls y n )}(x) \simeq
\varphi_{g(\mls y n )}(x)$.
...
\dfn $B$ je produktivní $\equiv \exists \varphi \in ČRF \st \forall x: W_x \in B \then \varphi(x) \da \& \varphi(x) \in B \ W_x$.
...
\obs{} Každá produktivní obsahuje nekonečnou $RSM$. 

\thm{O produktivní $ORF$} $\forall B$ produktivní $f \in ORF$.
...
\dfn $A,B$ jsou rekurzivně neoddělitelné $\equiv$ $\not \exists$ rekurzivní $M \st A \subseteq M \& B \subseteq \overline{M}$.
...
\thm{} Existují dvě efektivně neoddělitelné množiny.
$A = \{x | \varphi_x(x) \simeq 0 \}, B = \{ x | \varphi_x(x) \simeq 1 \}$.

\thm{Dvojná věta o rekurzi}

\thm{Ekvivalence $1$-úplnosti a efektivní neoddělitelnosti}
