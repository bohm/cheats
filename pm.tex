...
\lem{Lemma pro lemma} $X = Z_n$, zadefinuj $A_s = \{s,s+1,\dots s+k-1\}$ pro $s \in X$, $n$ buď
...
\prf{Slunečnicové lemma} Budeme volit náhodné $s$ a náhodnou $\sigma$, co
...
\dfn{} Definujme $n(k,l)$ jako maximální $n \st \exists \mls A n \andamp \mls B n \st$
\numlist\ndotted
\:$|A_i| = k, |B_i| = l,$
...
\thm{Bollobásovo lemma o průnicích} $n(k,l) = {k + l \choose k}$.

\prf{} Náhodně přepermutuj prvky. Zadefinuj jev: $U_i \equiv$ všechny prvky
...
\thm{Erdös, obvod neurčuje barevnost} $\forall k,l \exists G \st \chi(G) > k \andamp g(G) > l$.
...
\lem{Základní DRC} $G$ graf, $|G| = n, d$ prům. stupeň. Pokud existuje
...
\lem{O malém rozptylu.} Nechť $X_i$ je posloupnost proměnných, pro které platí $\lim_{n \rightarrow \infty} {\var(X_n) \over E[X_n]^2} = 0$.
...
\thm{Menší zlepšení rozdílných sum} $f(n) \le \log_2 n + (1/2) \log_2 \log_2 n + O(1)$.
...
\fsection{Lovászovo lokální lemma}
